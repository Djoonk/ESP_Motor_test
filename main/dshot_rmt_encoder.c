...
#ifndef __cplusplus
_Static_assert(sizeof(dshot_rmt_frame_t) == 0x02, "Invalid size of dshot_rmt_frame_t structure");
#endif
...
typedef struct
{
    ...
    int state;                            // the current encoding state
    bool bidirectional;                   // invert CRC in bidirectional mode
} dshot_rmt_encoder_t;

static void IRAM_ATTR make_dshot_frame(dshot_rmt_frame_t *frame, uint16_t throttle,
                                       bool telemetry, bool bidirectional)
{
                                   ...
                                   rmt_encode_state_t *ret_state)
{
    ...
    dshot_rmt_frame_t frame = {};
    make_dshot_frame(&frame, throttle->throttle, throttle->telemetry_req,
                     dshot_encoder->bidirectional);

    switch (dshot_encoder->state)
    {
    case 0: // send the dshot frame
        encoded_symbols += bytes_encoder->encode(bytes_encoder, channel, &frame,
                                                 sizeof(frame), &session_state);
