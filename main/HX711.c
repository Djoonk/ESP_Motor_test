...
static gpio_num_t GPIO_PD_SCK = GPIO_NUM_22;	
static gpio_num_t GPIO_DOUT = GPIO_NUM_23;		
...
void HX711_init(HX711_GAIN gain)
{
...
bool HX711_is_ready()
{
    return (gpio_get_level(GPIO_DOUT) == 0); // if 0 == 0 = true
}
...
{
	gpio_set_level(GPIO_PD_SCK, LOW);
	// wait for the chip to become ready (bounded to avoid infinite hang)
	int timeout_ms = 500;
	while (!HX711_is_ready())
	{
		...
		if (timeout_ms <= 0)
		{
			ESP_LOGE(DEBUGTAG, "HX711_read timeout: DOUT never went low");
			return 0;
		}
	}
...
{
	ESP_LOGD(DEBUGTAG, "===================== READ AVERAGE START ====================");
	unsigned long sum = 0;
	for (char i = 0; i < times; i++) 
	{
		sum += HX711_read();
	}
	ESP_LOGD(DEBUGTAG, "===================== READ AVERAGE END : %ld ====================",(sum / times));
	return sum / times;
}
...
esp_err_t HX711_zero( ) 
{
	ESP_LOGD(DEBUGTAG, "===================== START TARE ====================");
	unsigned long sum = 0; 
	sum = HX711_read_average(20);
	HX711_set_offset(sum);
	ESP_LOGD(DEBUGTAG, "===================== END TARE: %ld ====================",sum);
	return ESP_OK;
}
