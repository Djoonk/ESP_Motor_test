...
void esc_protocol_select(esc_protocol_t protocol)
{
    ...
    if (is_dshot_protocol(previous_protocol) && is_dshot_protocol(protocol))
    {
...
void esc_protocol_set_throttle(uint16_t throttle)
{
    ...
    {
        uint16_t pulse_us = ESC_MIN_US + ((ESC_MAX_US - ESC_MIN_US) * throttle) / 100U;
        esc_pwm_set_pulse_us(pulse_us);
        return;
    }

    uint16_t value = 48 + ((2047 - 48) * throttle) / 100U;
